...
def frob(Uold, Dsqold, Vold, U, Dsq, V):
    ...
    return np.abs(num) / max(denom, 1e-9) # Zikai: It was "return np.abs(num) / max(denom, 1e-9)".


class SoftImpute:
    def __init__(self, J=2, thresh=1e-05, lambda_=0, maxit=100, random_seed=None, verbose=False):
        self.J = J
        self.thresh = thresh
    ...
    def fit(self, X):
        ...
        while ratio > self.thresh and iters < self.maxit:
            ...
            if self.verbose:
                print('[softImpute] Iter %3d: ratio = %.5f' % (iters, ratio))
